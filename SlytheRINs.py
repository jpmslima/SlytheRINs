...
st.write('*SlytheRINs will compare the networks from the uploaded `.edges.txt` files and calculate various network parameters such as degree, betweenness centrality, clustering coefficient, triangle counts, and assortativity. The tool will also execute several statistical analyses to help you extract information from the effect of the conformational variation in the graph and the chemical interactions between residues. SlytheRINs output is a series of interactive plots. The plots and data are available for download in `.png` and `.tsv` formats.*')
...
if not am_df_final.empty:
    try:
        ...
        if am_df_final.empty:
            ...
            color_map_am = {'pathogenic': 'orange', 'benign': 'green', 'ambiguous': 'blue'} # User's preferred map
